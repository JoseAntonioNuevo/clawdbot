#!/bin/bash
# Wrapper to run OpenCode CLI with file-based output and timeout monitoring.
#
...
# Status file: $WORKDIR/.opencode-status.txt
#
# Status values:
...
PID_FILE="$WORKDIR/.opencode-pid.txt"
...
(
    ...
    " 2>&1 | \
        ...
        sed '/^$/d' | \
        cat > "$OUTPUT_FILE"
    echo "${PIPESTATUS[0]}" > "$WORKDIR/.opencode-exit-code.txt"
) &
MAIN_PID=$!
...
if [[ -f "$WORKDIR/.opencode-exit-code.txt" ]]; then
    EXIT_CODE=$(cat "$WORKDIR/.opencode-exit-code.txt")
    rm -f "$WORKDIR/.opencode-exit-code.txt"
else
    EXIT_CODE=1
fi
...
# Cleanup
rm -f "$PID_FILE" "$WORKDIR/.opencode-exit-code.txt"
